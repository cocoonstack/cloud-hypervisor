...
///
/// `dt_scan_depth1_nodes` in the arm64 kernel treats any depth-1 node other
/// than `chosen` as proof of a "real" DTB and turns ACPI off, so describing no
...
pub fn create_stub_fdt(
    ...
    fdt.property_u32("linux,uefi-secure-boot", 0)?;
...
mod tests {
    ...
    // The whole ACPI mode rests on this: dt_scan_depth1_nodes() in the arm64
    // kernel reads any depth-1 node other than `chosen` as proof of a real DTB
    ...
    fn stub_fdt_publishes_the_efi_handoff() {
        ...
        assert_eq!(u32_prop("linux,uefi-secure-boot"), 0);
    }
