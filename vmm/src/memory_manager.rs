    ...
    pub fn new_from_snapshot(
        ...
        if let Some(source_url) = source_url {
            ...
            if deltas.is_empty() {
                ...
                // Diff-snapshot series: fill from the baseline, then replay
                // each delta's dirty extents in sequence order; hole-punched
                // pages a delta never dirtied keep the content already
                // restored below them.
                if memory_restore_mode == MemoryRestoreMode::OnDemand {
