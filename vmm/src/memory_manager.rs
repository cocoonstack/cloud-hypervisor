...
if let Some((baseline, deltas)) = memory_chain.split_first() {
                // Diff-snapshot chain: load the baseline's full memory file
                // (honoring the restore mode), then replay each delta's dirty
                // extents in order; the source_url delta is the newest and
                // lands last. Replayed writes over a copy-on-write baseline
                // fault in privately, so the sharing story is unchanged.
                ...
                if memory_restore_mode == MemoryRestoreMode::CopyOnWrite {
                    mm_locked.mmap_cow_saved_regions(baseline, &mem_snapshot.memory_ranges)?;
                } else {
