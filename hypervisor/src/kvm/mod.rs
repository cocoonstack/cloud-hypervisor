...
// KVM_X86_REG_KVM(KVM_REG_GUEST_SSP): the guest's current shadow stack pointer. It is
// a register, not an MSR, so KVM_GET_MSRS never carries it.
#[cfg(target_arch = "x86_64")]
...
impl KvmVcpu {
    ...
    /// The guest's current shadow stack pointer, or `None` when the guest CPUID
    /// does not expose shadow stacks. A vCPU paused in user mode keeps its live SSP
    /// here, not in MSR_IA32_PL3_SSP, so without it a restored thread resumes with
    /// SSP=0 and faults on its next CALL/RET.
    ...
    /// Restore the guest's current shadow stack pointer.
